...
3. **编码**：当前实现把静态 PNG 编成两秒 MJPEG AVI，再套入 E01 表盘资源头。
4. **扫描和绑定**：只选择广播 `FD00` 的候选设备，完成 `0x60 → 0x61`。
5. **读取身份**：先确认尺寸、存储、协议和固件字段，不用外壳照片猜型号。
6. **认证与传输**：在白名单特征上执行 RCSP 认证和普通媒体更新命令。
...
当前版本的 `display` 仍是实验路径。一台实机在开始表盘更新时返回 `C5 reason 5`，所以不要把命令正常启动、BLE 写入回调或绑定成功当成完成。
